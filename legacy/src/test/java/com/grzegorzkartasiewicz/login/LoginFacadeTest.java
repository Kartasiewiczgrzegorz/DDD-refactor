...
class LoginFacadeTest {
    ...
    void shouldLogInUserWithCorrectCredentials() {
        // given
        Mockito.when(loginRepository.findByNick("testUser")).thenReturn(Optional.of(login));
        Mockito.when(userFacade.getUser(ArgumentMatchers.any(UserId.class))).thenReturn(userDTO);
    ...
    void shouldNotLogInNonExistentUser() {
        // given
        Mockito.when(loginRepository.findByNick(ArgumentMatchers.anyString())).thenReturn(Optional.empty());
