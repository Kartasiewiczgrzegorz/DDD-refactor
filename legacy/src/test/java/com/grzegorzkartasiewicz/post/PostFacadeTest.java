...
class PostFacadeTest {
    ...
    void shouldCorrectlyPrepareAndDelegateCommentCreation() {
        ...
        Mockito.verify(commentFacade).createComment(commentCreatorCaptor.capture());

        var capturedCommentCreator = commentCreatorCaptor.getValue();
        Assertions.assertThat(capturedCommentCreator.description()).isEqualTo(commentDescription);
        Assertions.assertThat(capturedCommentCreator.postId().id()).isEqualTo(postId);
        Assertions.assertThat(capturedCommentCreator.userId().id()).isEqualTo(authorDTO.getId());
    }
    ...
    void shouldPublishPostDeletedEventAndDeletePostById() {
        ...
        // then
        Mockito.verify(publisher).publish(ArgumentMatchers.any(PostDeletedEvent.class));
