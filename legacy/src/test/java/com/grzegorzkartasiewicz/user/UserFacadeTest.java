...
class UserFacadeTest {
    ...
    void shouldCorrectlyPrepareAndDelegatePostCreation() {
        ...
        Mockito.verify(postFacade).createPost(postCreatorCaptor.capture());

        var capturedPostCreator = postCreatorCaptor.getValue();
        Assertions.assertThat(capturedPostCreator.description()).isEqualTo(description);
        Assertions.assertThat(capturedPostCreator.userId().id()).isEqualTo(userId);
    }
}
