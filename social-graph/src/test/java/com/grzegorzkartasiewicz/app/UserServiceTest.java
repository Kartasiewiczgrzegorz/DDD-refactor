...
class UserServiceTest {
  ...
  void setUp() {
    requester = new User(new UserId(UUID.randomUUID()), new Name("John", "Doe"),
        new Email("john@example.com"), new HashSet<>(), new HashSet<>(), new HashSet<>(),
        new HashSet<>());
    target = new User(new UserId(UUID.randomUUID()), new Name("Jane", "Smith"),
        new Email("jane@example.com"), new HashSet<>(), new HashSet<>(), new HashSet<>(),
        new HashSet<>());
  }
  ...
  void sendFriendRequest_shouldAddRequestToBothUsersWhenValid() {
    ...
    // then
    assertThat(requester.isFriendRequestAlreadySent(target.getId())).isTrue();
    assertThat(target.isFriendRequestAlreadyReceived(requester.getId())).isTrue();
  ...
  void acceptFriendRequest_shouldEstablishFriendshipAndRemoveRequests() {
    ...
    // then
    assertThat(requester.isFriendRequestAlreadySent(target.getId())).isFalse();
    assertThat(target.isFriendRequestAlreadyReceived(requester.getId())).isFalse();

    assertThat(target.isFriend(requester.getId())).isTrue();
    assertThat(requester.isFriend(target.getId())).isTrue();
